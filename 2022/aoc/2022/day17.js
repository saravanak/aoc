...
function walkNodes(node, cost, directionStack, seenNodes) {
    ...
    if (node.x == nodes.length - 1 && node.y == nodes.length - 1) {
        console.log("found target node");
        return;
    }
    var endingDirection = directionStack.slice(-1);
    if (seenNodes.find(function (_a) {
        var x = _a.x, y = _a.y;
        return node.x == x && node.y == y;
    })) {
        console.log("Ignoring vistied node");
        return;
    }
    ...
    node.allowedDirections.forEach(function (d) {
        ...
        if (endingDirection == d && sameThreeDirs) {
            ...
            var nextCost = cost + node.path[d];
            nextNode.paths[d] = Math.min(nextNode.paths[d] || Number.POSITIVE_INFINITY, nextCost);
