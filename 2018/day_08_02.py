...
class Node:
    ...
    def compute_value(self):
        ...
        if self.child_node_count == 0:
            return sum(self.meta_data)
             
        child_values = 0     
        for i in self.meta_data:
            indexed_child = self.child_nodes[i-1] if i <= self.child_node_count else None
            if indexed_child:
                child_values += indexed_child.compute_value()
